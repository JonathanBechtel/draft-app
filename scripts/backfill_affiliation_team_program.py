"""Backfill ``player_affiliations.team_program_id`` from ``nba_team_id``.
...
and reports zero pending once caught up.
...
async def run_backfill(db: AsyncSession, *, dry_run: bool = False) -> BackfillReport:
    ...
    # pattern as scripts/populate_org_model_from_nba_teams.py's run_population).
    await db.rollback()
    async with db.begin():
