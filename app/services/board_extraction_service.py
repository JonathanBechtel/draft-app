...
def _substack_api_url(url: str) -> Optional[str]:
    ...
    Returns ``None`` for any URL that is not a Substack post (custom
    domains, non-Substack hosts, the bare ``substack.com`` apex, or
    Substack URLs that don't follow the ``/p/<slug>`` post pattern). The
    API URL drops any query string — the JSON endpoint doesn't accept
    one and including it returns 404.
    """
    ...
    # A post URL is "/p/<slug>" — after rstrip+split that's ["", "p", "<slug>"].
    if len(path_parts) < 3 or path_parts[1] != "p" or not path_parts[2]:
        return None
    slug = path_parts[2]
