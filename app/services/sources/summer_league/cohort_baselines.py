...
* ``debut`` — one data point per player: their single, chronologically
  **earliest qualifying individual game** within ``season_range`` (its raw
  GmSc via :func:`~app.services.sources.summer_league.metrics.game_score_from_row`),
...
def qualifying_game_values(
    ...
    distribution. Reuses :func:`~app.services.sources.summer_league.metrics.game_score_from_row`
    ...
    *,
    ...
    Args:
        rows: ``(log_row, game_date)`` pairs -- ``log_row`` exposes
            ...
            box-score fields :func:`~app.services.sources.summer_league.metrics.game_score_from_row`
...
async def build_baselines(
    ...
    *,
    ...
    ``app.services.sources.summer_league.metrics.rebuild``).
