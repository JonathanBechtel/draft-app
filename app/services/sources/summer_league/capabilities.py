...
:mod:`app.services.sources.summer_league.normalization` already owns and sets -- onto the canonical
...
* :func:`row_provides` / :func:`rows_provide` infer a ``provides`` set directly from a fetched
  ...
  :func:`app.services.sources.summer_league.normalization.normalize_pbp_events`), so the row's own field
...
# see app.services.sources.summer_league.normalization.normalize_pbp_events.
    ...
    *,
    ...
    Args:
        pbp_available: ``SummerLeagueEdition.pbp_available`` -- at least one game in this
            ...
            :func:`app.services.sources.summer_league.normalization.normalize_pbp_events`).
        shotchart_available: ``SummerLeagueEdition.shotchart_available`` -- at least one
            game has parsed shot-location rows (see
            :func:`app.services.sources.summer_league.normalization.normalize_shot_events`).
        adv_eligible: Whether this pool cleared the box-completeness threshold that makes
            ...
            :mod:`app.services.sources.summer_league.metrics`. Not owned by ``normalization.py``, so
...
def competition_capability_provides(
    ...
    Turns the availability flags :mod:`app.services.sources.summer_league.normalization`
    ...
    :mod:`app.services.sources.summer_league.metrics`), so a caller that has already
...
def _row_has_pbp_fields(r: Any) -> bool:
    ...
    ``app.services.sources.summer_league.metrics._season_columns``), so either being non-``None`` is
