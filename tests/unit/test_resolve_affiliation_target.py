"""Unit coverage for the dual-read affiliation-target resolution helper.

No DB round-trip: constructs bare ``PlayerAffiliation`` instances (never
flushed) and asserts the resolution rule -- prefer ``team_program_id``, fall
back to ``nba_team_id``, and return ``None`` when neither target is known.
"""
