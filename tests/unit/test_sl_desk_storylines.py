...
from app.services.sources.summer_league import desk_storylines as storylines
from app.services.sources.summer_league.desk_facts import GameLine, PriorEvent
...
def _grade(
    *,
    pctl: float,
    cohort_key: str = "status:undrafted",
    gated: bool = False,
    value: float = 10.0,
) -> GradeRow:
    return GradeRow(
...
def test_base_weight_covers_exactly_five_trigger_types() -> None:
    ...
    assert {t.value for t in BASE_WEIGHTS} == {
        t.value for t in SummerLeagueDeskTriggerType
    }
...
def test_detect_debut_number_one_pick_outranks_number_forty_five() -> None:
    ...
    top = ProspectSlot(
        player_id=1, player_label="Top", draft_round=1, draft_pick=1, consensus_rank=1
    )
    deep = ProspectSlot(
        player_id=2,
        player_label="Deep",
...
def test_detect_duel_positive_two_prominent_prospects_fire() -> None:
    a = ProspectSlot(
        player_id=1, player_label="A", draft_round=1, draft_pick=1, consensus_rank=1
    )
    b = ProspectSlot(
        player_id=2, player_label="B", draft_round=1, draft_pick=2, consensus_rank=2
    )
...
def test_detect_duel_near_miss_only_one_prospect_clears_cutoff() -> None:
    ...
    b = ProspectSlot(
        player_id=2, player_label="B", draft_round=2, draft_pick=20, consensus_rank=20
    )
...
def test_detect_duel_top_pair_outranks_two_second_rounders() -> None:
    ...
    late_a = ProspectSlot(
        player_id=3, player_label="C", draft_round=2, draft_pick=10, consensus_rank=13
    )
    late_b = ProspectSlot(
        player_id=4, player_label="D", draft_round=2, draft_pick=11, consensus_rank=14
    )
...
# --------------------------------------------------------------------------- #
def _streak_games(
    pctls: list[float], value: float = 20.0, median: float = 10.0
) -> list[GameLine]:
...
def test_detect_streak_near_miss_only_two_games_does_not_fire() -> None:
    subject = ProspectSlot(
        player_id=1, player_label="ShortRun", draft_round=1, draft_pick=1
    )
...
def test_detect_status_heat_positive_undrafted_above_85th() -> None:
    subject = ProspectSlot(
        player_id=1, player_label="Sleeper", draft_round=None, draft_pick=None
    )
...
def test_detect_status_heat_near_miss_lottery_pick_ineligible() -> None:
    ...
    subject = ProspectSlot(
        player_id=1, player_label="Lottery", draft_round=1, draft_pick=1
    )
...
def test_detect_second_look_positive_returner_with_notable_swing() -> None:
    subject = ProspectSlot(
        player_id=1, player_label="Sophomore", draft_round=1, draft_pick=5
    )
...
def test_detect_second_look_near_miss_small_swing_does_not_fire() -> None:
    subject = ProspectSlot(
        player_id=1, player_label="Steady", draft_round=1, draft_pick=5
    )
    prior = PriorEvent(year=2025, value=10.0, gp=5)
    inst = detect_second_look(
...
def test_detect_second_look_near_miss_debutant_has_no_prior() -> None:
    ...
    subject = ProspectSlot(
        player_id=1, player_label="Rookie", draft_round=1, draft_pick=5
    )
    inst = detect_second_look(
...
# --------------------------------------------------------------------------- #
def _instance(
    trigger: SummerLeagueDeskTriggerType, weight: float, realized: float | None = None
) -> TriggerInstance:
...
def test_rank_slate_live_finals_sink_below_in_progress() -> None:
    ...
    live_low_weight = GameSlateInput(
        ...
        tip_datetime=None,
        instances=[
            _instance(SummerLeagueDeskTriggerType.STATUS_HEAT, 60.0, realized=1.0)
        ],
    )
...
def test_realized_deviation_from_pctl_90th_and_10th_both_score_40() -> None:
    ...
    assert (
        realized_deviation_from_pctl(90.0) == realized_deviation_from_pctl(10.0) == 40.0
    )
...
def test_max_realized_deviation_tie_between_90th_and_10th_resolves_deterministically() -> (
    None
):
...
def test_rank_slate_live_falls_back_to_entering_weight_when_live_deviation_missing() -> (
    None
):
...
def test_rank_slate_live_tie_on_live_deviation_breaks_deterministically_on_game_id() -> (
    None
):
    ...
    assert (
        first == second == [3, 5]
    )  # lower game_id wins the tie, regardless of input order
...
def test_select_quiet_slate_hero_prefers_ungated_candidates() -> None:
    candidates = [
        ClassLeaderCandidate(
            player_id=1, player_label="Confident", pctl=80.0, gmsc=15.0, gated=False
        ),
        ClassLeaderCandidate(
            player_id=2, player_label="Thin", pctl=99.0, gmsc=25.0, gated=True
        ),
    ]
    hero = select_quiet_slate_hero(candidates)
    assert hero is not None
...
def test_select_quiet_slate_hero_falls_back_to_gated_when_all_gated() -> None:
    ...
    candidates = [
        ClassLeaderCandidate(
            player_id=1, player_label="Only", pctl=60.0, gmsc=10.0, gated=True
        ),
    ]
    hero = select_quiet_slate_hero(candidates)
    assert hero is not None
...
def test_module_source_has_no_competitive_or_contract_language() -> None:
    ...
    for term in _BANNED_TERMS:
        assert term not in code_only, (
            f"banned term {term!r} found in desk_storylines.py"
        )
