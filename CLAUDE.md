...
This repo uses a four-skill chain to take a feature from idea to merged code: `/create-product-pitch` (the "why") → tech spec → `/create-qa-checklist` (observable behaviors + test plan) → `/create-project` (GitHub Project with orchestrator-ready tickets) → `/orchestrate` (parallel-agent execution). The pitch is optional for mechanical work but recommended for novel features where scope or rationale matters; the other three steps are standard.
- **Per-repo overrides live in `docs/plans/ai-orchestrator-ticket-spec.md`.** That file is the source of truth for ticket-template overrides, browser-verification recipes, conda env, dev-server command, and screenshot output paths. Agents working on this repo should read it before producing tickets or running browser verification.
- **Dev admin credentials** for orchestrated browser verification live in `.env` (gitignored) as `DRAFTGURU_ADMIN_EMAIL` / `DRAFTGURU_ADMIN_PASSWORD`. A dedicated test account (`admin@draftguru.local`) exists so orchestrator runs don't muddle the repo owner's personal sessions. Never embed these values in tickets, PR descriptions, screenshots, or commits.
- **`gh` auth gotcha:** the env-var `GH_TOKEN` lacks the `project` scope. Prefix orchestrator-related `gh` calls with `unset GH_TOKEN && ...` so the keyring credential (with `project` scope) is used.
- **Verification flavors** generated tickets use: unit / integration / e2e (browser via Playwright MCP or repo harness) / visual (`make visual`). UI tickets default to integration + e2e + visual; backend tickets default to unit + integration. The `/verify` built-in skill is the recommended driver for live-app verification.
