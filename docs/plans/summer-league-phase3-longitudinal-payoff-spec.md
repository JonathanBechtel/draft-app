...
- **D1 — Backfill, don't wait.** The trend axis is synthesized for historical events (2017–2026) by computing cumulative-through-day lines from `summer_league_player_game_logs` via the shared engine and publishing them as daily-close versions (`as_of` = event day, `published_at` set, `is_current=False`). This matches compaction's retention grain exactly — going forward, live rebuilds + compaction produce the same shape organically. One idempotent operator script in `scripts/`; the read path cannot tell backfilled from organic rows.
...
- **Read service** (`app/services/` — engine-agnostic contract): `get_daily_trend(scope_key, player_id | cohort, metric_keys, event_window) -> series of (as_of, value, cohort_band)`, selecting the latest published version per (scope, day) — precisely compaction's daily-close invariant. Registry metric keys only.
- **Backfill script** (`scripts/backfill_sl_daily_trend_versions.py`): per historical event day, compute through-day lines from game logs via the shared engine, publish as non-current daily versions through the existing publish path (which enforces scoping + staleness guards). Idempotent on `(scope, version)`; re-run safe; respects the writer lock via the bounded acquire. Runs after #732. Dev first, prod via runbook note.
...
| T4 | Trend read service + API endpoint (scope-parameterized, registry keys, daily-close selection) | T1 | unit, integration | new `app/services/` module, route, `app/models/` shapes |
| T5 | Historical backfill script + measured row counts + runbook note (**gated on #732 merged**) | T4 | integration (docker PG), idempotency test | new `scripts/` module |
...
3. The trend surface renders from retained history on player and event pages, with cohort context and honest `as_of` labeling.
